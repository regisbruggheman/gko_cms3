...
module Gko
  class FileUtilz
    ...
    # A general purpose method to unzip a directory (+source+) into a destination
    # directory, including all files and subdirectories.
    # If 'force' is true destination directory is deleted before unzip (if exist!). 
    def self.unzip_file(file, destination, force=true)
      if force && File.exist?(destination) then
