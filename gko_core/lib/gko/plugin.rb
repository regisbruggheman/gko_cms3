module Gko
  ...
  class Plugin
    ...
    attr_accessor :name, :class_name, :controller, :directory, :url, :icon,
                  :version, :dashboard, :always_allow_access, :menu_match,
                  :hide_from_menu, :pathname, :plugin_activity, :in_settings
    ...
    def url
      @url ||= if controller.present?
        { :controller => "gko/admin/#{controller}" }
      elsif directory.present?
        { :controller => "gko/admin/#{directory.split('/').pop}" }
      else
        { :controller => "gko/admin/#{name}" }
      end
